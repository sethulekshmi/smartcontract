...
{	
	...
	tracing.create('ENTER', 'GET blockchain/assets1/assets/asset/'+assetID+'/stamp', {});
	configFile = reload(__dirname+'/../../../../../../configurations/configuration.js');
	...
	request(options, function(error, response, body)
	{
		...
		if (!error && !body.hasOwnProperty('error') && response.statusCode == 200)
		{
			...
			result.message = asset.date;
			tracing.create('EXIT', 'GET blockchain/assets1/assets/asset/'+assetID+'/stamp', result);
			res.send(result)
		}
		else 
		{
			res.status(400)
			var error = {}
			error.message = 'Unable to read stamp'
			error.assetID = assetID;
			error.error = true;
			tracing.create('ERROR', 'GET blockchain/assets1/assets/asset/'+assetID+'/stamp' , error)
			res.send(error)
		}
	});
}
