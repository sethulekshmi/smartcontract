...
{	
	...
	tracing.create('ENTER', 'GET blockchain/assets1/assets/asset/'+assetID+'/diamondat', {});
	configFile = reload(__dirname+'/../../../../../../configurations/configuration.js');
	...
	request(options, function(error, response, body)
	{
		if (!error && !body.hasOwnProperty("error") && response.statusCode == 200)
		{
			...
			result.message = asset.diamondat;
			tracing.create('EXIT', 'GET blockchain/assets1/assets/asset/'+assetID+'/diamondat', result);
			res.send(result)
		}
		else 
		{
			res.status(400)
			var error = {}
			error.message = 'Unable to read diamondat'
			error.assetID = assetID;
			error.error = true;
			tracing.create('ERROR', 'GET blockchain/assets1/assets/asset/'+assetID+'/diamondat', error)
			res.send(error)
		}
	});
}
