...
{	
	...
	tracing.create('ENTER', 'GET blockchain/assets1/assets/asset/'+assetID+'/location', {});
	configFile = reload(__dirname+'/../../../../../../configurations/configuration.js');
	...
	request(options, function(error, response, body)
	{
		...
		if (!error && !body.hasOwnProperty("error") && response.statusCode == 200)
		{
			...
			result.message = asset.location;
			tracing.create('EXIT', 'GET blockchain/assets1/assets/asset/'+assetID+'/location', result);
			res.send(result)
		}
		else 
		{
			...
			error.error = true;
			tracing.create('ERROR', 'GET blockchain/assets1/assets/asset/'+assetID+'/location' , error)
			res.send(error)
		}
	});
}
