...
{	
	...
	tracing.create('ENTER', 'GET blockchain/assets1/assets/asset/'+assetID+'/clarity', {});
	configFile = reload(__dirname+'/../../../../../../configurations/configuration.js');
	...
	request(options, function(error, response, body)
	{
		...
		if (!error && !body.hasOwnProperty("error") && response.statusCode == 200)
		{
			...
			result.message = asset.clarity;
			tracing.create('EXIT', 'GET blockchain/assets1/assets/asset/'+assetID+'/clarity' , result);
			res.send(result)
		}
		else 
		{
			res.status(400)
			var error = {}
			error.message = 'Unable to read clarity'
			error.assetID = assetID;
			error.error = true;
			tracing.create('ERROR', 'GET blockchain/assets1/assets/asset/'+assetID+'/clarity' , error)
			res.send(error)
		}
	});
}
